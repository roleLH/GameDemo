...
-- 切换场景：内部使用协程
local function CoInnerSwitchScene2(self, scene_config)
	-- 打开loading界面
	local uimgr_instance = UIManager:GetInstance()
	uimgr_instance:OpenWindow(UIWindowNames.UILoading)
	local window = uimgr_instance:GetWindow(UIWindowNames.UILoading)
...
-- 切换场景
local function SwitchSameScene(self, scene_config)
	assert(scene_config ~= LaunchScene and scene_config ~= LoadingScene)
	assert(scene_config.Type ~= nil)
	if self.busing then 
		return
	...
	self.busing = true
	coroutine.start(CoInnerSwitchScene2, self, scene_config)
end
...
SceneManager.SwitchSameScene = SwitchSameScene
